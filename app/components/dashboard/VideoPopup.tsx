...
const StoryPopup: React.FC<StoryPopupProps> = ({
  ...
  const [activeSubtitleWordIndex, setActiveSubtitleWordIndex] =
    useState<number>(-1);
  ...
  const currentStepThumbnail =
    currentStep?.image ??
    storyData?.thumbnailUrl ??
    storyData?.story?.[0]?.image ??
  ...
  const handleAudioEnded = useCallback(() => {
    ...
    if (isLooping) {
      // Story loop is active
      if (currentStepIndex === totalSteps - 1) {
        // If it's the last step of the story
        ...
        // Not the last step, but story loop is on
        setCurrentStepIndex((prev) => prev + 1); // Go to the next step
      }
    } else {
      // Story loop is NOT active
      if (currentStepIndex < totalSteps - 1) {
  ...
  const handleAudioVolumeChange = useCallback(() => {
    if (audioRef.current) setIsAudioMuted(audioRef.current.muted);
  }, []);
  ...
  const handleAudioPlayingEvent = useCallback(() => {
    setIsAudioLoading(false);
  ...
  const showStepChangeLoader =
    isAudioLoading && !initialLoadComplete && !audioDuration;

  const [isEnded, setIsEnded] = useState(false);

  useEffect(() => {
    if (currentStepIndex === totalSteps - 1 && isAudioPlaying) {
      setIsEnded(true);
    }
  }, [currentStepIndex, totalSteps, isAudioPlaying]);

  return (
    <div
      ref={containerRef}
      className="fixed inset-0 z-50 bg-black flex flex-col text-white font-sans"
    >
      {/* Header */}
      ...
      <div className="absolute top-0 left-0 right-0 z-30 flex items-center justify-between p-4 md:p-6 bg-gradient-to-b from-black/60 to-transparent">
        <div className="flex items-center gap-3 overflow-hidden">
          ...
          <h2 className="text-lg md:text-xl font-semibold truncate whitespace-nowrap">
            {storyTitle}
          </h2>
        </div>
        <button
      ...
      {/* Main Content Area */}
      <div className="flex-grow flex flex-col md:flex-row pt-16 md:pt-20 px-4 md:px-8 lg:px-40 md:pb-24 overflow-hidden">
        {" "}
        {/* Adjusted padding */}
        {/* Left Side: Image */}
        <div
        ...
        >
          <img
            ...
            className={`object-contain w-full h-full ${
              isFullscreen ? "rounded-none" : "rounded-lg md:rounded-2xl"
            }`}
          ...
          {isFullscreen && (
            <div className="absolute bottom-0 left-0 w-full px-4 pb-8 flex justify-center pointer-events-none">
              <div
                className="bg-black/80 text-white text-xl md:text-2xl px-6 py-3 rounded-lg mb-4 pointer-events-auto"
                style={{ maxWidth: "90vw" }}
              >
                <span>
        ...
        {!isFullscreen && (
          <div className="w-full mt-4 md:mt-0 md:w-1/2 lg:w-2/4 h-1/2 md:h-full flex flex-col p-4 md:py-6 md:pr-4 lg:pr-10 md:pl-4 lg:pl-6 order-2 md:order-2 overflow-y-auto custom-scrollbar">
            {" "}
            {/* Adjusted padding and added custom-scrollbar */}
            <div className="relative flex-grow flex flex-col">
              <div className="relative z-10 mt-8 md:mt-16">
                {" "}
                {/* Added margin-top to push content below bg text */}
                {isEnded && !isAudioPlaying ? (
                  <>
                    <EndScreen />
                  </>
                ) : (
      ...
      <div className="absolute bottom-0 left-0 right-0 z-30 px-4 py-3 bg-gradient-to-t from-black/70 to-transparent">
            ...
            if (clickedStepIndex >= 0 && clickedStepIndex < totalStorySteps) {
              if (currentStepIndex !== clickedStepIndex) {
                ...
                if (audioDuration > 0) {
                  ...
                  const stepEndPercent =
                    (clickedStepIndex + 1) / totalStorySteps;
                  const percentWithinStep =
                    (percent - stepStartPercent) /
                    (stepEndPercent - stepStartPercent);
                  audioRef.current.currentTime =
                    percentWithinStep * audioDuration;
        ...
        <div className="flex items-center justify-between text-white">
          ...
          <div className="flex items-center gap-2 w-1/4 opacity-80 hover:opacity-100 transition-opacity">
            <img
              src={currentStepThumbnail}
              alt="Current step thumbnail"
          ...
          <div className="flex items-center gap-x-1 sm:gap-x-2 md:gap-x-3 justify-center">
            ...
            <button
              ...
              disabled={
                isAudioLoading && !audioDuration && !initialLoadComplete
              } // Disable if loading AND no duration AND not initially complete
          ...
          <div className="flex items-center gap-x-1 sm:gap-x-2 md:gap-x-3 justify-end w-1/4">
            ...
            <button
              ...
              className="flex items-center gap-1 sm:gap-1.5 p-1.5 sm:p-2 hover:bg-white/10 rounded-full"
            >
