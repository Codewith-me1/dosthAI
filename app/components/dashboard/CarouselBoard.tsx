...
import Card from "./Card"; // Assuming Card component exists and accepts a 'type' prop
...
const CarouselBoard: React.FC = () => {
  ...
  const [selectedOption, setSelectedOption] = useState("story"); // For creation radio buttons
      ...
      <div className="relative group">
        ...
        <div className="overflow-hidden">
          <Swiper
            ...
            className="!pb-10" // Added ! to ensure override if necessary
          >
            {items.map((item) => (
              <SwiperSlide className="max-w-[20rem]" key={item.id}>
                <Card
                  author="BCBA" // Assuming author is static or comes from elsewhere
                  key={String(item.id)} // Card itself should have a key if mapped, but SwiperSlide has it here
                  id={String(item.id)}
                  title={item.title}
                  image={item.image}
                  rating={item.rating}
                  category={item.category} // This is the sub-category
                  type="story" // Correctly passed type
                />
              </SwiperSlide>
            ))}
          </Swiper>
        ...
        <div>
          ...
          <div className="space-y-12">
            {categories.stories.map((subCategory) => (
              <div key={subCategory}>
                {/* Sub-category title H3 is removed here */}
    ...
    <div className="max-w-[120rem] mx-auto">
      ...
      <div className="text-center mb-8">
        ...
        <p className="text-gray-600 text-sm">
          Create your first story with a simple prompt
        </p>
      </div>
