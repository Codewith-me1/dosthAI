  ...
  return (
    <div className=" w-full bg-white ">
      <div className="w-full flex flex-col items-center justify-center pt-8 pb-8">
        {/* Breadcrumb */}
        <div className="w-full px-4 text-md text-gray-500 mb-2">
          Explore / Create a Story
        </div>
        {/* Main Heading */}
        <h1 className="text-2xl sm:text-3xl font-semibold text-center mb-2">
          Lets create a <span className="text-[#7B2FF2]">story</span>
        </h1>
        <p className="text-center text-gray-500 mb-6 max-w-lg mx-auto text-base font-normal">
          Create your first story, write a simple prompt! For example.
          <br />
          <span className="font-semibold text-black">
            Create a story about how to walk my dog
          </span>
        </p>
        {/* Toggle */}
        <div className="flex justify-center gap-6 mb-6">
          {["Story", "Activity", "Cards"].map((type) => (
            <button
              ...
              className={`flex items-center gap-1 px-4 py-1.5 rounded-full border text-sm font-medium transition-all ${
                selectedType === type
                  ? "bg-[#F3F0FF] text-[#7B2FF2] border-[#7B2FF2] shadow"
          ...
          <div className="flex gap-4 mb-2">
            {characters.map((char, idx) => (
              <button
                ...
                className={`flex flex-col items-center gap-1 px-2 py-1 rounded-lg border-2 ${
              ...
              >
                {char.img ? (
                  ...
                  <span className="w-12 h-12 flex items-center justify-center rounded-lg bg-gray-100 border-2 border-dashed border-gray-300 text-2xl text-gray-400">
                    +
                  </span>
                )}
      ...
      <CreateCharacterModal
        ...
        onSave={(character) => {
          setCharacters((prev) => [
            ...prev,
            { name: character.name, img: character.image },
          ]);
